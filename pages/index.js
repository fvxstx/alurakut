...
export default function Home(props) {
  ...
  React.useEffect(() => {
    // API Git
    fetch(`https://api.github.com/users/fvxstx/followers`).then(async (res) => {
    ...
    <>
      ...
      <MainGrid>
        ...
        <div className="welcomeArea" style={{ gridArea: "welcomeArea" }}>
          <Box>
            <h1 className="name">Bem vindo(a)</h1>
...
export async function getServerSideProps(context) {
  ...
  const { isAuthenticated } = await fetch(
    "http://alurakut-eight-pi.vercel.app/api/auth",
    {
      headers: {
        Authorization: token,
      },
